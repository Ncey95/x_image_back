...
import com.ncey95.x_image_back.api.imagesearch.ImageSearchApiFacade;
...
import com.ncey95.x_image_back.manager.auth.annotation.SaSpaceCheckPermission;
import com.ncey95.x_image_back.manager.auth.model.SpaceUserPermissionConstant;
...
public class PictureController {
    ...
    // 分页获取图片封装VO 包含用户信息 用户使用的接口 获取图片列表 并缓存到本地缓存中
    @GetMapping("/get/vo")
...
//            }
    ...
    // 分页获取图片封装VO 包含用户信息 用户使用的接口 获取图片列表 并缓存到本地缓存中
    @Deprecated // 已废弃 请使用 listPictureVOByPage 方法
    ...
    // 以图搜图 调用图片搜索接口 以图片url为参数 返回搜索结果
